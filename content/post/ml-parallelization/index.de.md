...
In einigen Domänen wie z.B. der natürlichen Sprachverarbeitung können die Eingabedaten sehr lange Sequenzen enthalten, wie z.B. lange Dokumente oder Audiodaten, was bei einigen Modellarchitekturen zu Speicherproblemen führt. So hat beispielsweise der Transformer [@vaswaniAttentionAllYou2017] eine quadratische Zeit- und Speicherkomplexität in Bezug auf die Eingabelänge [@childGeneratingLongSequences2019].
...
@liSequenceParallelismLong2022 führen eine Form der Sequenzparallelisierung ein, bei der die Aktivierungen des Aufmerksamkeitsmoduls des Transformers anhand der Längendimension partitioniert werden. Hierdurch wird die Berechnung der Aufmerksamkeit im Transformer auch für längere Sequenzen ermöglicht. Da diese jedoch jedes Token in der Sequenz mit jedem anderen Token in Relation setzt, müssen die Aktivierungen zwischen den Knoten ausgetauscht werden. Die Autoren schlagen dazu *Ring Self Attention* vor, bei der die partitionierten Key- und Value-Embeddings zwischen allen Rechenknoten zirkulieren. Bei der Verteilung auf $D$ Rechenknoten kann mit diesem Ansatz zwar der Speicheraufwand um den Faktor $\mathcal{O}(D^2)$ reduziert werden, doch wird die Berechnung der Aufmerksamkeit um den Faktor $\mathcal{O}(D)$ langsamer[^speed-seq1].

[^speed-seq1]: *Ring Self Attention* verlangsamt zwar die Berechnung der Aufmerksamkeit um den Faktor $D$, doch bedeutet dies nicht, dass Sequenz-Parallelisierung nach @liSequenceParallelismLong2022 zwangsweise einen negativen Einfluss auf den Trainingsdurchsatz hat. Die Verringerung des für die Aktivierungen benötigten Speichers kann auch die Erhöhung der Batch-Größe ermöglichen, sodass mehr Trainingspaare auf einmal berechnet werden können, was sich wiederum positiv auf den Trainingsdurchsatz auswirkt.

@korthikantiReducingActivationRecomputation2022 ergänzen die tensor-parallele Berechnung des Transformers nach @shoeybiMegatronLMTrainingMultiBillion2020 um einer Parallelisierung der LayerNorm- und Dropout-Operatoren. Dazu werden die Aktivierungen derer entlang der Sequenz-Dimension verteilt, sodass der für die Aktivierungen nötige Speicher pro Rechenknoten merklich reduziert werden kann. Bei der Vorwärtsberechnung wird vor dem Aufmerksamkeitsmechanismus eine AllGather-Operation entlang der Sequenzdimension durchgeführt ($f$ in der Grafik in [Tensorparallelisierung](#tensor-parallelisierung)). Bei der Rückwärtsberechnung ist an dieser Stelle eine ReduceScatter-Operation notwendig. Da AllReduce-Operationen häufig als AllGather und ReduceScatter implementiert werden, entspricht das Kommunikationsvolumen dem der Tensorparallelisierung nach @shoeybiMegatronLMTrainingMultiBillion2020. In der Praxis hat sich die Kombination von spaltenweiser Tensor-Parallelisierung mit Sequenz-Parallelisierung nach @korthikantiReducingActivationRecomputation2022 bewährt, da sie eine kompromisslose Verringerung es nötigen Speichervolumens ermöglicht.

- @jacobsDeepSpeedUlyssesSystem2023
- @liLightSeqSequenceLevel2023: Sequence-level parallelism for distributed training of long context transformers?? 🚧
