...
@korthikantiReducingActivationRecomputation2022 ergänzen die tensor-parallele Berechnung des Transformers nach @shoeybiMegatronLMTrainingMultiBillion2020 um einer Parallelisierung der LayerNorm- und Dropout-Operatoren. Dazu werden die Aktivierungen entlang der Sequenz-Dimension verteilt, sodass der für die Aktivierungen nötige Speicher pro Rechenknoten merklich reduziert werden kann. Bei der Vorwärtsberechnung wird vor dem Aufmerksamkeitsmechanismus eine AllGather-Operation entlang der Sequenzdimension durchgeführt ($f$ in der obigen Grafik). Bei der Rückwärtsberechnung ist an dieser Stelle eine ReduceScatter-Operation notwendig. 🚧🚧🚧. Da AllReduce-Operationen häufig als AllGather und ReduceScatter implementiert werden, entspricht das Kommunikationsvolumen dem der Tensorparallelisierung nach @shoeybiMegatronLMTrainingMultiBillion2020. Sie nennen diese Form der Parallelisierung zwar Sequenzparallelisierung, doch hat diese Methode außer dem Namen kaum Gemeinsamkeiten mit der von @liSequenceParallelismLong2022 vorgestellten Sequenzparallelisierung. In der Praxis hat sich die Kombination von spaltenweiser Tensor-Parallelisierung mit Sequenzparallelisierung bewährt, da sie zueinander komplementär sind und der geringere Speicherverbrauch entweder größere Batch-Größen ermöglicht, die wiederum den Durchsatz erhöhen, oder das Training überhaupt erst möglich machen.
